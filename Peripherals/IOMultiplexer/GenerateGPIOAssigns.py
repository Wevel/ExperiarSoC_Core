...
def GenerateVerilog(pinMap: list[tuple[int, str, str]], outputPath: str):
	...
	for pin in pinMap:
		...
		if type.upper() == "INPUT":
			...
			elif name.startswith("CACHEDMEMORY") and name.count('_') == 1:
				nameParts = name.split('_')
				pinType = nameParts[-1]
				index = getIndex(name[12])
				if index >= 0 and pinType == "CSB" or pinType == "SCK":
					writeCachedMemoryPin(outputLines, name, index, True, pinType, allowGPIO)
				else:
					print(f"Invalid CachedMemory pin index for pin 'IO{pinNumber}-{name}'")
			elif name.startswith("UART"):
				index = getIndex(name[4:-3])
			...
			if name.startswith("CACHEDMEMORY") and name.count('_') == 1:
				nameParts = name.split('_')
				pinType = nameParts[-1]
				index = getIndex(name[12])
				if index >= 0 and pinType == "IO0" or pinType == "IO1":
					writeCachedMemoryPin(outputLines, name, index, False, pinType, allowGPIO)
				else:
					print(f"Invalid CachedMemory pin index for pin 'IO{pinNumber}-{name}'")
			else:
				writeUndefinedPin(outputLines, name, True, allowGPIO)
...
def writeCachedMemoryPin(outputLines: list[str], pinName: str, memoryIndex: int, isOutput: bool, pinType: str, allowGPIO: bool):
	if allowGPIO:
		if isOutput:
			...
			outputLines.append(f"	assign io_out[PIN_{pinName}] = cachedMemory_en[{memoryIndex}] ? cachedMemory_{pinType.lower()}[{memoryIndex}] : gpio_output[PIN_{pinName}];\n")
			outputLines.append(f"	assign io_oeb[PIN_{pinName}] = cachedMemory_en[{memoryIndex}] ? {IO_OUTPUT} : gpio_oe[PIN_{pinName}];\n")
		else:
			outputLines.append(f"	assign gpio_input[PIN_{pinName}] = cachedMemory_en[{memoryIndex}] ? 1'b0 : (gpio_oe[PIN_{pinName}] ? io_in[PIN_{pinName}] : 1'b0);\n")
		...
		if isOutput:
			outputLines.append(f"	assign gpio_input[PIN_{pinName}] = 1'b0;\n")
			outputLines.append(f"	assign io_out[PIN_{pinName}] = cachedMemory_{pinType.lower()}[{memoryIndex}];\n")
			outputLines.append(f"	assign io_oeb[PIN_{pinName}] = {IO_OUTPUT};\n")
		else:
			outputLines.append(f"	assign gpio_input[PIN_{pinName}] = 1'b0;\n")
			outputLines.append(f"	assign io_out[PIN_{pinName}] = cachedMemory_{pinType.lower()}_write[{memoryIndex}];\n")
			outputLines.append(f"	assign io_oeb[PIN_{pinName}] = !cachedMemory_{pinType.lower()}_we[{memoryIndex}];\n")
			outputLines.append(f"	assign cachedMemory_{pinType.lower()}_read[{memoryIndex}] = !cachedMemory_{pinType.lower()}_we[{memoryIndex}] ? io_in[PIN_{pinName}] : 1'b0;\n")
...
def main():
	pinMap = [
	    ...
	    (7, "Input", "GPIO/IRQ"),
	    (8, "Output", "GPIO/CachedMemory0_CSB"),
	    (9, "Output", "GPIO/CachedMemory0_SCK"),
	    (10, "InOut", "GPIO/CachedMemory0_IO0"),
	    (11, "InOut", "GPIO/CachedMemory0_IO1"),
	    (12, "Output", "GPIO/CachedMemory1_CSB"),
	    (13, "Output", "GPIO/CachedMemory1_SCK"),
	    (14, "InOut", "GPIO/CachedMemory1_IO0"),
	    (15, "InOut", "GPIO/CachedMemory1_IO1"),
	    (16, "Output", "GPIO/PWM0"),
	    (17, "Output", "GPIO/PWM1"),
	    (18, "Output", "GPIO/PWM2"),
	    (19, "Output", "GPIO/PWM3"),
